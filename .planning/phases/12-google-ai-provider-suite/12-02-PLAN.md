...
- Constructor takes `api_key: str` and `output_dir: str`. Calls `genai.configure(api_key=api_key)`. Creates images_dir.
- `generate_image()`:
  - Use `genai.ImageGenerationModel("imagen-4.0-generate-preview")` (note: the deprecated SDK uses `ImageGenerationModel` from `google.generativeai`)
  - IMPORTANT: Check SDK availability. The deprecated `google-generativeai` SDK may not expose `ImageGenerationModel` directly. If not available, use the Gemini model approach instead: `model = genai.GenerativeModel("gemini-2.5-flash")` with image generation config. The research shows Imagen can be accessed through the Gemini API.
  - Alternative approach if direct ImageGenerationModel is unavailable: Use `model = genai.GenerativeModel("imagen-4.0-generate-preview")` and call `model.generate_images(prompt=prompt, config={"number_of_images": num_images, "aspect_ratio": aspect_ratio})` where aspect_ratio is derived from width/height.
  - Handle reference_images: if provided, load each with `PIL.Image.open(path)` and pass to generate_images.
  - Save generated images to output_dir with unique filenames.
  - Wrap API call in tenacity retry: `@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=2, min=4, max=30))`
  - Fallback: if API call fails, log error and fall back to MockImageProvider
- `supports_resolution()`: Check against Imagen supported sizes. Imagen supports various aspect ratios but images are typically generated at standard sizes. Return True for common sizes (512-2048 range).
  </action>
  <verify>
Run:
```python
