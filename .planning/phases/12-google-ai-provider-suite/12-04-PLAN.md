...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/config.py"
      provides: "Updated video_provider_type comment with veo option"
      contains: "mock/svd/kling/minimax/veo"
  key_links:
...
<task type="auto">
  <name>Task 2: Refactor trend_analyzer.py and update requirements/env/config</name>
  <files>
    app/services/trend_analyzer.py
    requirements.txt
    .env.example
...
**app/config.py** - Update video_provider_type comment:
- Change line 45's comment from `# mock/svd/kling/minimax` to `# mock/svd/kling/minimax/veo` to document veo as a valid video provider option
- This is a comment-only change; no code logic changes needed
  </action>
  <verify>
Run:
```python
...
Should return 1.
  ...
  <done>trend_analyzer.py uses LLMProvider abstraction. requirements.txt includes google-generativeai for Python 3.9. .env.example documents GOOGLE_API_KEY, LLM_PROVIDER_TYPE, IMAGE_PROVIDER_TYPE, and updated VIDEO_PROVIDER_TYPE with veo option. app/config.py video_provider_type comment updated to include veo.</done>
</task>
