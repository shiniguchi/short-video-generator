...
- `generate_structured()`: Use Gemini's native JSON mode:
  ...
  - Set `generation_config = {"temperature": temperature, "response_mime_type": "application/json", "response_schema": schema}` (NOTE: use `response_schema` parameter with Pydantic class directly -- the google-generativeai SDK accepts Pydantic models natively for `response_schema`)
